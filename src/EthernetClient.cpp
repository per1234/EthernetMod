...
uint16_t EthernetClient::_timeoutDuration = 1000;      //default timeout
...
int EthernetClient::connect(IPAddress ip, uint16_t port) {
  ...
  while (status() != SnSR::ESTABLISHED) {
    delay(1);
    if (status() == SnSR::CLOSED || millis() - start > _timeoutDuration) {
      _sock = MAX_SOCK_NUM;
      return 0;
    }
  }
...
void EthernetClient::stop() {
  ...
  while (status() != SnSR::CLOSED && millis() - start < _timeoutDuration)
    delay(1);
...
void EthernetClient::setClientTimeout(uint16_t timeoutDuration) {
